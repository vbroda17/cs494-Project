...
void alloc_key_buff( void )
{
    ...
    #pragma omp parallel for schedule(runtime)  // changed from nothing
...
#ifdef SCHED_CYCLIC
    #pragma omp parallel for private(i,j,k,k1) schedule(runtime)    // changed from static,1
#else
    #pragma omp parallel for private(i,j,k,k1) schedule(runtime)    // changed from dynamic
#endif
...
#pragma omp parallel private(i,j,k,k1,k2)
  {
    #pragma omp for schedule(runtime)   // changed from nothing, there was no schedule specifier here
    for( i=0; i<NUM_KEYS; i++ )
...
/*  Confirm keys correctly sorted: count incorrectly sorted keys, if any */
    ...
    j = 0;
    #pragma omp parallel for schedule(runtime) reduction(+:j)     // changed from nothing
...
/*  Determine the number of keys in each bucket */
    #pragma omp for schedule(static)    // when changed to runtime this broke logic/semantics
    for( i=0; i<NUM_KEYS; i++ )
...
/*  Sort into appropriate bucket */
    #pragma omp for schedule(static)    // when changed to runtime this also broke semantics
    for( i=0; i<NUM_KEYS; i++ )  
    {
        k = key_array[i];
...
#ifdef SCHED_CYCLIC
    #pragma omp for schedule(runtime) // changed from static, 1
#else
    #pragma omp for schedule(runtime)   // changed from dynamic
#endif
    for( i=0; i< NUM_BUCKETS; i++ ) {
...
/*  In this section, the keys themselves are used as their 
    ...
    #pragma omp for nowait schedule(runtime) // changed from static
    for( i=0; i<NUM_KEYS; i++ )
